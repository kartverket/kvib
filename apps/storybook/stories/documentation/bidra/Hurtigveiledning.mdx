...
<Meta title="Info/Bidra/Hurtigveiledning" />
...
Repoet inneholder både kode for komponentpakka og dokumentasjon på storybook. For å jobbe på komponenter i pakka må du inn i `packages/react/src/<komponent>`. For dokumentasjon på storybook er det `apps/storybook/stories/components/<komponent>` som gjelder.
...
- For detaljerte gjennomganger for utvikling av komponenter, se:
  ...
  > [Bygging av komponent](/docs/info-bidra-bygge--docs)

  > [Styling av komponent](/docs/info-bidra-style--docs)

  > [Dokumentasjon av komponent](/docs/info-bidra-dokumentere--docs)

- Ta en titt på [Kjente Problemer](/docs/info-bidra-kjente-problemer--docs) for å unngå å gjøre de samme feilene som andre har gjort før deg.
...
Sjekk ut den detaljerte beskrivelsen av [Testing](/docs/info-bidra-teste--docs) for mer informasjon.
...
1. Dersom du har gjort endringer på et komponent må du legge til et changeset som oppdaterer pakken:
   ...
   > Sjekk ut [Publish](/docs/info-bidra-publish--docs) hvis du vil vite mer om hvordan dette fungerer.
