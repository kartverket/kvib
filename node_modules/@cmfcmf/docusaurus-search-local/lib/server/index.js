...
const optionsSchema = utils_validation_1.Joi.object({
    ...
    maxSearchResults: utils_validation_1.Joi.number().integer().min(1).default(8),
    lunr: utils_validation_1.Joi.object({
        ...
        tagsBoost: utils_validation_1.Joi.number().min(0).default(3),
        parentCategoriesBoost: utils_validation_1.Joi.number().min(0).default(2),
    }).default(),
});
function cmfcmfDocusaurusSearchLocal(context, options) {
    let { indexDocSidebarParentCategories, indexBlog, indexDocs, indexPages, language, style, maxSearchResults, lunr: { tokenizerSeparator: lunrTokenizerSeparator, k1, b, titleBoost, contentBoost, tagsBoost, parentCategoriesBoost, }, } = options;
    ...
    return {
        ...
        async postBuild({ routesPaths = [], outDir, baseUrl, siteConfig: { trailingSlash }, plugins, }) {
            ...
            const documents = (await Promise.all(data.map(async ({ file, url, type }) => {
                ...
                const docusaurusTag = (0, parse_1.getDocusaurusTag)(html);
            ...
            logger_1.default.info(`${Object.keys(documentsByDocusaurusTag).length} indexes will be created.`);
            await Promise.all(Object.entries(documentsByDocusaurusTag).map(async ([docusaurusTag, documents]) => {
                logger_1.default.info(`Building index ${docusaurusTag} (${documents.length} documents)`);
                const index = lunr(function () {
                    ...
                    const that = this;
                    documents.forEach(({ id, sectionTitle, sectionContent, sectionTags, docSidebarParentCategories, }) => {
                ...
                await writeFileAsync(path_1.default.join(outDir, `search-index-${docusaurusTag}.json`), JSON.stringify({
                ...
                logger_1.default.info(`Index ${docusaurusTag} written to disk`);
            }));
        },
    };
}
